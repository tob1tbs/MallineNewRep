...
class BuilderController extends Controller
{
    ...
    public function actionBuilderIndex() {
        if(Auth::check()) {
            ...
            return redirect()->route('actionMainIndex')->withError('login');            
        }
    }

    public function actionBuilderSuccess() {
        if(Auth::check()) {
            if (view()->exists('builder.builder_success')) {

                $data = [
                ];
                
                return view('builder.builder_success', $data);
            } else {
                abort('404');
            }
        } else {
